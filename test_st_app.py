...
from st_app import get_model_size

class TestDashboardRenderingLogic(unittest.TestCase):
    
    @patch('os.path.exists')
    @patch('os.path.getsize')
    def test_get_model_size_baseline(self, mock_getsize, mock_exists):
        """Test that get_model_size returns accurate physical size for baseline."""
        # Setup mock file size to be exactly 64 MB (64 * 1024 * 1024 bytes)
        mock_exists.return_value = True
        mock_getsize.return_value = 64 * (1024 ** 2)
        
        # Test baseline logic
        size = get_model_size('rtdetr-l.pt')
    ...
    @patch('os.path.exists')
    @patch('os.path.getsize')
    def test_get_model_size_optimised(self, mock_getsize, mock_exists):
        """Test that get_model_size returns accurate physical size for optimised model."""
        # Setup mock file size to be exactly 34.5 MB
        mock_exists.return_value = True
        mock_getsize.return_value = 34.5 * (1024 ** 2)
        
        # Test optimized logic
        size = get_model_size('prismnet_optimised.pt')
    ...
    @patch('os.path.exists')
    def test_get_model_size_missing_file(self, mock_exists):
        """Test fallback when model file is missing."""
        mock_exists.return_value = False
        
        size = get_model_size('missing_file.pt')
